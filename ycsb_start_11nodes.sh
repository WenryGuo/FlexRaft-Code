...
# Encoding mode (runtime gflag for ycsb_server_multiraft).
# 取值: RS_F | RS_3F | LRC
...
echo "========================================"
pkill -9 -f ycsb_server_multiraft 2>/dev/null || true
...
BINARY="${BUILD_DIR}/bench/ycsb_server_multiraft"
CONF="${SCRIPT_DIR}/conf/multi-raft-11-localtest.conf"
...
for i in "${NODES[@]}"; do
    ...
    "$BINARY" --conf "$CONF" --id "$i" --peer_threads=14 --apply_threads 14 --batch_size 32 \
              --encoding="$ENCODING" > "$LOG_FILE" 2>&1 &
    PID=$!
    echo "Node $i started (PID: $PID, encoding=${ENCODING})"
done
...
while [ $elapsed -lt $MAX_WAIT ]; do
    ...
    printf "\r  Elapsed: %3ds | Running: %d/11 | KV Ports listening: %d/11" \
        "$elapsed" "$running" "$listening"

    if [ "$running" -eq 11 ] && [ "$listening" -eq 11 ]; then
        echo ""
        echo ""
...
if [ $crash_count -eq 0 ]; then
    echo "SUCCESS: All 11 nodes started and ready! (encoding=${ENCODING})"
    exit 0
else
